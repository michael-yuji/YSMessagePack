...
public extension NSData {
    ...
    ///
    /// - parameter amount:               Specify the amount of items to unpack. If amount is `nil`, the function will unpack all items automatically.
    /// - parameter returnRemainingBytes: if `true`, bytes not unpacked will add to the end of the array.
    ///
    /// - throws: If any error occurs.
    ///
    /// - returns: An `NSData` array with unpacked data, if `return_remainingBytes` is set to `true`, bytes not unpacked will add to the end of array
    ...
    /// Unpack data to an array of unpacked `NSData`, with an array of type enum associated with data unpacked.
    ///
    /// - parameter amount:               Specify the amount of items to unpack. If amount is `nil`, the function will unpack all items automatically.
    /// - parameter returnRemainingBytes: if `true`, bytes not unpacked will add to the end of the array.
    ///
    /// - throws: If any error occurs.
    ///
    /// - returns: An `NSData` array with unpacked data, and an array of types associated with the type of data unpacked.
    ...
    /// Unpack data to an array of duple which contains the data and its type
    ///
    /// - parameter amount:               Specify the amount of items to unpack. If amount is `nil`, the function will unpack all items automatically.
    /// - parameter returnRemainingBytes: if `true`, bytes not unpacked will add to the end of the array.
    ///
    /// - throws: If any error occurs.
    ///
    /// - returns: An array of duple `(NSData, DataTypes)` with unpacked data
    ...
    private static func parsePackedArray(_ bytes: NSData, index: Int, count: Int, length: inout Int) ->  NSData?
    {
        ...
        return (data_buffer == nil) ? nil : (NSKeyedArchiver.archivedData(withRootObject: data_buffer! as NSArray) as NSData)
    }
    ...
    private static func parsePackedMap(_ bytes: NSData, index: Int, count: Int, length: inout Int)   ->  NSData?
    {
        ...
        return (data_buffer == nil) ? nil : (NSKeyedArchiver.archivedData(withRootObject: dict! as NSDictionary) as NSData)
    }
     ...
     */
    public typealias UnpackHandler = (_ unpackedData: [NSData], _ isLast: Bool) -> Bool
    ...
    */
    public func unpackByGroupsWith(objectsInEachGroup: Int, numberOfSetsToUnpack setsCount: Int? = nil, handlerForEachGroup handler: UnpackHandler) throws {
        ...
        while (i < length) {
                ...
                count += 1
                if !handler(dataArray, (i == length) ||  count == setsCount) || count == setsCount {
                    break
                }
...
//    private func unpack(startIndex i_: Int = 0, specific_amount amount: Int? = nil, return_remainingBytes: Bool = false, dataLengthOutput len: inout Int) throws -> ([NSData], [DataTypes])
    {
        ...
        while (i < byte_array.count) {
            ...
            let _fixStrDataMarkupSize    =  Int(byte_array[i] ^ 0b101_00000)/MemoryLayout<UInt8>.size
            let _fixArrayDataCount       =  Int(byte_array[i] ^ 0b1001_0000)/MemoryLayout<UInt8>.size
            let _fixMapCount             =  Int(byte_array[i] ^ 0b1000_0000)/MemoryLayout<UInt8>.size
...
public extension NSData {
    ...
    public func unpackAsyncForEach(priority: dispatch_queue_priority_t, amount: Int? = nil, competitionHandler: @escaping (_ data: NSData, _ type: DataTypes, _ atIndex: Int) -> Void) {
            ...
            if let parsed = parsed {
                ...
                for (index, s) in parsed.enumerated() {
                    que.async(group: group, qos: priority, flags: DispatchWorkItemFlags(rawValue: UInt(0)), execute: {
                        ...
                        competitionHandler(data, s.1, index)
                    })
                }
            }
        })
    ...
    public func unpackAsync(priority: dispatch_queue_priority_t, amount: Int? = nil, competitionHandler: @escaping ([NSData]) -> Void ) {
